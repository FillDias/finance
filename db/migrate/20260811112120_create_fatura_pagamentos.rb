class CreateFaturaPagamentos < ActiveRecord::Migration[7.2]
  def change
    create_table :fatura_pagamentos do |t|
      t.references :cartao, null: false, foreign_key: true
      t.date :mes_referencia, null: false
      t.decimal :valor_pago, precision: 10, scale: 2, null: false
      t.date :data_pagamento, null: false

      t.timestamps
    end

    add_index :fatura_pagamentos, [ :cartao_id, :mes_referencia ], unique: true
  end
end
